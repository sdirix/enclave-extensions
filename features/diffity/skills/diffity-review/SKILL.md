...
2. Tell the user the review is ready and they can check the session's VNC viewer (the browser runs inside the container; open it on the host with `enclave vnc-viewer`). Example:

   > Review complete — check the session's VNC viewer.
   >
